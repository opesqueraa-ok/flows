...
function defaultState(){
  return {
    ...
    lastWakeTime: null, // recuerda la última hora que pusiste en "¿A qué hora despertar?"
    moreSubTab: 'cuidado', // sub-pestaña activa dentro de "Más": cuidado | enfoque | notes
    ...
    syncCode: null,        // código de sincronización con Firebase (null = no sincroniza)
    lastSavedAt: null,     // timestamp (ms) del último guardado local, para decidir quién gana al sincronizar
    averagesDisplayMode: 'duration' // 'duration' = cuánto tardo | 'clock' = a qué hora lo hago
  };
}
...
function saveState(){
  ...
  if (state.syncCode) scheduleCloudPush();
}

/* ---------------------------------------------------------------------- */
/* Sincronización en la nube (Firebase, opcional)                          */
/* ---------------------------------------------------------------------- */
// No usa login: el "código de sincronización" ES la credencial — un texto
// largo y aleatorio que actúa como el nombre del documento en Firestore.
// Quien tenga el código puede leer/escribir esos datos, así que se genera
// automáticamente con suficiente longitud para ser imposible de adivinar.

const FIREBASE_SDK_VERSION = '10.13.0';
let __fb = null;          // { db, doc, getDoc, setDoc, serverTimestamp }
...
function generateSyncCode(){
  ...
  // Respaldo por si el navegador no soporta randomUUID (poco común)
  let s = '';
...
async function loadFirebase(){
  if (__fb) return __fb;
  if (!window.firebaseConfig || window.firebaseConfig.apiKey === 'PON_AQUI_TU_apiKey'){
    throw new Error('Falta configurar firebase-config.js con los datos de tu proyecto.');
  }
  ...
  __fb = { db: getFirestore(app), doc, getDoc, setDoc, serverTimestamp };
  return __fb;
}

function scheduleCloudPush(){
  clearTimeout(cloudPushTimer);
  cloudPushTimer = setTimeout(pushToCloud, 2000); // agrupa varios cambios seguidos en un solo envío
}
...
async function pushToCloud(){
  ...
  try{
    ...
    const ref = fb.doc(fb.db, 'flows_backups', state.syncCode);
    await fb.setDoc(ref, { data: JSON.stringify(state), updatedAt: fb.serverTimestamp(), savedAt: state.lastSavedAt });
    syncStatus = 'synced';
    syncErrorMsg = '';
  }catch(err){
    syncStatus = 'error';
    syncErrorMsg = err.message || 'No se pudo sincronizar';
  }
  if (currentView === 'summary') renderSyncSection();
}

// Trae lo que haya en la nube y decide quién gana. Si forcePull es true
// (por ejemplo, al conectar un código que ya trae datos de otro dispositivo),
// la nube siempre gana — no tendría sentido comparar fechas contra un
// dispositivo nuevo que casi no tiene historial propio todavía.
async function pullFromCloud(silent = false, forcePull = false){
  if (!state.syncCode) return;
  syncStatus = 'loading';
  if (!silent && currentView === 'summary') renderSyncSection();
  try{
    const fb = await loadFirebase();
    const ref = fb.doc(fb.db, 'flows_backups', state.syncCode);
    ...
    if (snap.exists()){
      ...
      const cloudSavedAt = cloud.lastSavedAt || 0;
      const localSavedAt = state.lastSavedAt || 0;
      if (forcePull || cloudSavedAt > localSavedAt){
        const code = state.syncCode; // conservar el código aunque se reemplace todo lo demás
        state = Object.assign(defaultState(), cloud, { syncCode: code });
        localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
        if (!silent) toast('Datos actualizados desde la nube');
        renderCurrentView();
      } else {
        await pushToCloud(); // lo local está más al día — sube esa versión
      }
    } else {
      await pushToCloud(); // no había nada en la nube todavía — primera vez
    }
    syncStatus = 'synced';
    syncErrorMsg = '';
  }catch(err){
    syncStatus = 'error';
    syncErrorMsg = err.message || 'No se pudo conectar con la nube';
  }
  if (currentView === 'summary') renderSyncSection();
}

function enableSync(code, forcePull = false){
  state.syncCode = code;
  saveState();
  pullFromCloud(false, forcePull);
}
...
function renderSyncSection(){
  ...
  if (!state.syncCode){
    el.innerHTML = `
      <p class="card-sub" style="margin-bottom:10px;">Guarda tu historial también en la nube (Firebase), para no perderlo si actualizas la app o cambias de dispositivo. No pide cuenta ni contraseña — usa un código largo y aleatorio que solo tú vas a tener.</p>
      <button class="btn-primary full" id="btn-generate-sync">Generar código de sincronización</button>
      <p class="card-sub" style="margin:10px 0 4px;">¿Ya tienes un código de otro dispositivo?</p>
      <div style="display:flex; gap:8px;">
        <input id="sync-code-input" class="text-input" placeholder="Pega tu código aquí">
        <button class="btn-secondary small" id="btn-connect-sync">Conectar</button>
      </div>
    `;
    document.getElementById('btn-generate-sync').addEventListener('click', ()=>{
      const code = generateSyncCode();
      enableSync(code, false);
    });
    document.getElementById('btn-connect-sync').addEventListener('click', ()=>{
      ...
      if (!val){ toast('Pega un código primero'); return; }
      enableSync(val, true);
    });
    return;
  }
  ...
  const statusColor = { idle:'var(--text-faint)', loading:'var(--teal-blue)', synced:'var(--teal-blue)', error:'var(--danger)' }[syncStatus];

  el.innerHTML = `
    <div class="stat-row"><span>Estado</span><span class="stat-val" style="color:${statusColor};">${statusLabel}</span></div>
    ${syncStatus==='error' ? `<p class="card-sub" style="color:var(--danger);">${escapeHtml(syncErrorMsg)}</p>` : ''}
    <p class="card-sub" style="margin:10px 0 4px;">Tu código (para conectar otro dispositivo — trátalo como una contraseña):</p>
    <div class="card-sub" style="font-family:monospace; background:var(--surface-soft); padding:8px 10px; border-radius:8px; word-break:break-all; user-select:all;">${state.syncCode}</div>
    <div class="mini-btn-row" style="margin-top:10px;">
      <button class="pill-btn" id="btn-copy-sync">Copiar código</button>
      <button class="pill-btn" id="btn-resync">↻ Sincronizar ahora</button>
    </div>
    <button class="text-btn danger" id="btn-disable-sync" style="margin-top:10px;">Dejar de sincronizar</button>
  `;
  document.getElementById('btn-copy-sync').addEventListener('click', ()=>{
    navigator.clipboard?.writeText(state.syncCode);
    toast('Código copiado');
  });
  document.getElementById('btn-resync').addEventListener('click', ()=> pullFromCloud());
  document.getElementById('btn-disable-sync').addEventListener('click', ()=>{
    if (confirm('Esto deja de sincronizar este dispositivo. Tus datos aquí no se borran. ¿Continuar?')) disableSync();
  });
}
...
// IDs de actividades de la rutina que ya tienen un registro hoy (hecho u
// omitido — no importa el orden en que quedaron en el arreglo de logs).
// Usar un set en vez de la posición permite borrar cualquier entrada sin
// que la app se confunda sobre cuál es el siguiente paso.
function handledActivityIds(session){
  return new Set(session.logs.filter(l => !l.custom).map(l => l.activityId));
}

function getNextActivity(routine, session){
  const handled = handledActivityIds(session);
  return routine.activities.find(a => !handled.has(a.id)) || null;
}
...
function routineProgress(session){
  return handledActivityIds(session).size;
}
...
function logNextActivity(skip = false){
  ...
  if (!routine) return;
  const activity = getNextActivity(routine, session);
  if (!activity) return;
  ...
  if (handledActivityIds(session).size === routine.activities.length){
...
function deleteLogEntry(sessionId, index){
  ...
  const routine = getRoutine(session.routineId);
  session.completed = routine ? handledActivityIds(session).size === routine.activities.length : false;
  saveState();
}
...
// Promedio de la hora del día (reloj) a la que sueles hacer esta actividad —
// distinto del promedio de duración: aquí promediamos el momento del día,
// no cuánto tardó el paso.
function averageClockTime(routineId, activityId){
  const minutesList = [];
  state.sessions.forEach(s => {
    if (s.routineId !== routineId) return;
    s.logs.forEach(log => {
      if (log.custom || log.activityId !== activityId || log.excluded || log.skipped || !log.time) return;
      const d = new Date(log.time);
      minutesList.push(d.getHours()*60 + d.getMinutes() + d.getSeconds()/60);
    });
  });
  if (!minutesList.length) return null;
  const avgMin = minutesList.reduce((a,b)=>a+b,0) / minutesList.length;
  const result = new Date();
  result.setHours(Math.floor(avgMin/60) % 24, Math.round(avgMin%60), 0, 0);
  return result;
}

// "Reinicia" el promedio de una actividad marcando todo su historial pasado
// como excluido — así el promedio vuelve a calcularse desde cero con lo que
// registres de ahora en adelante. No borra tus registros de días pasados,
// solo deja de tomarlos en cuenta para el cálculo (se pueden reincluir uno
// por uno después con el botón ⦸/↺ de cada día si hiciera falta).
function resetAverageForActivity(routineId, activityId){
  state.sessions.forEach(s => {
    if (s.routineId !== routineId) return;
    s.logs.forEach(log => {
      if (!log.custom && log.activityId === activityId) log.excluded = true;
    });
  });
  saveState();
}
...
const views = ['home','routines','sleep','more','summary'];
const moreSubViews = ['cuidado','enfoque','notes'];
...
// Fuerza cuál sub-vista de "Más" se ve, tocando el estilo inline además de
// la clase CSS — así funciona aunque el navegador tenga guardada una
// versión vieja de style.css en caché.
function applyMoreSubVisibility(sub){
  moreSubViews.forEach(v => {
    const el = document.getElementById(`sub-${v}`);
    const isActive = v === sub;
    el.classList.toggle('active', isActive);
    el.style.display = isActive ? 'block' : 'none';
  });
  document.querySelectorAll('#more-subnav [data-sub]').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.sub === sub);
  });
}

function switchMoreSubView(name){
  state.moreSubTab = name;
  saveState();
  applyMoreSubVisibility(name);
  renderMoreSubView();
}

function renderMoreSubView(){
  const sub = state.moreSubTab || 'cuidado';
  if (sub === 'cuidado') renderCuidado();
  else if (sub === 'enfoque') renderEnfoque();
  else if (sub === 'notes') renderNotes();
}
...
function renderCurrentView(){
  if (currentView === 'home') renderHome();
    ...
    applyMoreSubVisibility(state.moreSubTab || 'cuidado');
    renderMoreSubView();
  }
}
...
function renderHomeIdle(el, greet, session){
  ...
  if (gotoCuidadoBtn) gotoCuidadoBtn.addEventListener('click', ()=> { switchMoreSubView('cuidado'); switchView('more'); });
...
/* ---- Planificación (vive en la pestaña Rutinas: cálculo puntual + horario fijo por día) ---- */

function renderRoutinePlanning(){
  const el = document.getElementById('routine-planning');
  if (!el) return;
  const routine = activeRoutine();
  ...
  el.innerHTML = `
    <div class="card" style="margin-top:14px;">
      ...
      <p class="card-sub" style="margin-bottom:10px;">Dinos a qué hora quieres completar el último paso y calculamos hacia atrás la hora ideal para cada actividad, de tu rutina activa (${routine ? escapeHtml(routine.name) : 'ninguna'}).</p>
      <div style="display:flex; gap:8px;">
    ...
    <div class="card" style="margin-top:14px;">
      ...
      <p class="card-sub" style="margin-bottom:10px;">Ej.: los lunes debes salir de casa a las 5:00 AM, otro día a las 6:00 AM — configúralo una vez y todo se recalculará solo cada vez que abras la app ese día. Aparecerá arriba en Inicio.</p>
...
function renderHomeActive(el, greet, session){
  const routine = getRoutine(session.routineId);
  const nextActivity = getNextActivity(routine, session);
  ...
  if (viewMode === 'all'){
    ...
    const pendingRows = routine.activities.filter(a => !handled.has(a.id) && a.id !== nextActivity.id).map(a => `
...
function renderLoggedRow(session, index, opts = {}){
  ...
  return `
    <div class="timeline-item">
      ...
      ${!log.custom ? `<button class="small-icon-btn" title="${log.excluded ? 'Incluir en el promedio' : 'Excluir del promedio'}" data-toggle-exclude="${index}">${log.excluded ? '↺' : '⦸'}</button>` : ''}
      <button class="small-icon-btn" title="Eliminar" data-del-entry="${index}">✕</button>
    </div>
    ...
    if (sleepMode === 'A'){
      html += `
        <div class="card">
          ...
          <input type="time" id="wake-time-input" class="text-input" value="${state.lastWakeTime || defaultTimeValue(7,0)}">
...
function wireSleepEvents(){
  ...
  if (calcBtn) calcBtn.addEventListener('click', ()=>{
    ...
    document.getElementById('btn-start-tracking').addEventListener('click', ()=>{
      // sleepTime = el momento en que apretaste "seguir esta noche" — es tu
      // "ya me voy a dormir", para poder medir cuánto dormiste al final.
      const session = { id: uid(), mode:'A', targetWake: wake.toISOString(), sleepTime: new Date().toISOString(), status:'sleeping', cyclesPlanned:5 };
      state.sleepSessions.push(session);
  ...
  if (wakeBtn) wakeBtn.addEventListener('click', ()=>{
    ...
    s.cyclesActual = Math.max(0, Math.round(((now - new Date(s.sleepTime)) - FALL_ASLEEP_MIN*60000) / (CYCLE_MIN*60000)));
    s.status = 'done';
...
function startFocusTicker(){
  ...
  focusTickHandle = setInterval(()=>{
    ...
    if (!elEl || currentView !== 'more' || state.moreSubTab !== 'enfoque') return;
...
function renderSummary(){
  ...
  if (routine && routine.activities.length){
    const avgMode = state.averagesDisplayMode || 'duration';
    const avgRows = routine.activities.map((a,i)=>{
      if (i===0) return '';
      const hasAnyData = averageDuration(routine.id, a.id) != null;
      let valueText;
      if (avgMode === 'clock'){
        const avgClock = averageClockTime(routine.id, a.id);
        valueText = avgClock != null ? fmtTimeShort(avgClock) : 'Sin datos aún';
      } else {
        const avg = averageDuration(routine.id, a.id);
        valueText = avg != null ? fmtDuration(avg) : 'Sin datos aún';
      }
      return `
        <div class="stat-row">
          <span>${escapeHtml(a.name)}</span>
          <span style="display:flex; align-items:center; gap:8px;">
            <span class="stat-val">${valueText}</span>
            ${hasAnyData ? `<button class="small-icon-btn" title="Reiniciar promedio" data-reset-avg="${a.id}">↺</button>` : ''}
          </span>
        </div>`;
    }).join('');
    html += `
      <div class="card">
        <div class="card-row" style="margin-bottom:8px;">
          <div class="card-title">Tus promedios — ${escapeHtml(routine.name)}</div>
        </div>
        <div class="mode-toggle" id="averages-mode-toggle" style="margin-bottom:10px;">
          <button data-avg-mode="duration" class="${avgMode==='duration'?'active':''}" style="padding:8px 0; font-size:12.5px;">Tiempo</button>
          <button data-avg-mode="clock" class="${avgMode==='clock'?'active':''}" style="padding:8px 0; font-size:12.5px;">Horas</button>
        </div>
        ${avgRows}
      </div>`;
  }
  ...
  if (lastSleep){
    const sleptMs = (lastSleep.sleepTime && lastSleep.wakeTime)
      ? new Date(lastSleep.wakeTime) - new Date(lastSleep.sleepTime) : null;
    html += `
      <div class="card">
        ...
        ${sleptMs!=null ? `<div class="stat-row"><span><strong>Tiempo dormido</strong></span><span class="stat-val">${fmtDuration(sleptMs)}</span></div>` : ''}
  ...
  // Este es el arreglo clave: sin estas dos líneas, el toggle Horas/Tiempos
  // y los botones de editar/excluir/eliminar se veían pero no hacían nada.
  if (todaySession && todaySession.logs.length){
    wireTimelineModeToggle(renderSummary);
    wireLoggedRowEvents(todaySession, renderSummary);
  }

  document.querySelectorAll('#averages-mode-toggle [data-avg-mode]').forEach(b=>b.addEventListener('click', ()=>{
    state.averagesDisplayMode = b.dataset.avgMode;
    saveState();
    renderSummary();
  }));
  document.querySelectorAll('[data-reset-avg]').forEach(b=>b.addEventListener('click', ()=>{
    if (confirm('Esto borra el historial de esta actividad del cálculo del promedio (empieza de cero). No borra tus registros del día a día, solo deja de contarlos para el promedio. ¿Continuar?')){
      resetAverageForActivity(routine.id, b.dataset.resetAvg);
      toast('Promedio reiniciado');
      renderSummary();
    }
  }));
...
document.querySelectorAll('#more-subnav [data-sub]').forEach(btn=>{
  btn.addEventListener('click', ()=> switchMoreSubView(btn.dataset.sub));
});
...
switchView('home');
if (getActiveSleep()) startNightTicker();
if (state.syncCode) pullFromCloud(true);
