...
function switchMoreSubView(name){
  ...
  moreSubViews.forEach(v => {
    document.getElementById(`sub-${v}`).classList.toggle('active', v === name);
  });
...
function renderCurrentView(){
  if (currentView === 'home') renderHome();
    ...
    const sub = state.moreSubTab || 'cuidado';
    moreSubViews.forEach(v => document.getElementById(`sub-${v}`).classList.toggle('active', v === sub));
    document.querySelectorAll('#more-subnav [data-sub]').forEach(btn => btn.classList.toggle('active', btn.dataset.sub === sub));
    renderMoreSubView();
  }
}
